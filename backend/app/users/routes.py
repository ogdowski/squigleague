...
from app.users.auth import get_google_oauth_client, get_discord_oauth_client
...
async def oauth_google_callback(
    ...
    try:
        ...
        frontend_url = settings.CORS_ORIGINS[0] if settings.CORS_ORIGINS else "http://localhost"
        return RedirectResponse(url=f"{frontend_url}/oauth/callback?token={access_token}&provider=google")
...
async def oauth_discord_callback(
    ...
    try:
        ...
        if oauth_account:
            ...
            if not user:
                # Create new user
                username = email.split('@')[0] if email else f"discord_{provider_user_id[:8]}"
                # Make username unique if needed
                counter = 1
                base_username = username
                while session.exec(select(User).where(User.username == username)).first():
        ...
        # Redirect to frontend with token
        frontend_url = settings.CORS_ORIGINS[0] if settings.CORS_ORIGINS else "http://localhost"
        return RedirectResponse(url=f"{frontend_url}/oauth/callback?token={access_token}&provider=discord")
