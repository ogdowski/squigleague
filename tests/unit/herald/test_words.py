...
class TestGenerateExchangeID:
    ...
    def test_generate_exchange_id__correct_format(self):
        ...
        assert all(c in '0123456789abcdef' for c in mini_hash), f"Invalid hex chars in {mini_hash}"
    ...
    def test_generate_exchange_id__fallback_after_max_attempts(self):
        ...
        parts = exchange_id.split('-')
        assert len(parts) == 4
        assert len(parts[3]) == 6, f"Fallback hash should be 6 chars, got {len(parts[3])}"
...
class TestValidateExchangeID:
    ...
    def test_validate_exchange_id__invalid_format_returns_false(self):
        ...
        for exchange_id in invalid_ids:
            assert not validate_exchange_id(exchange_id), f"Should reject: {exchange_id}"
    ...
    def test_validate_exchange_id__validates_generated_ids(self):
        ...
        for _ in range(20):
            exchange_id = generate_exchange_id(check_exists_callback=lambda x: False)
            assert validate_exchange_id(exchange_id), f"Generated ID should validate: {exchange_id}"
...
class TestWordLists:
    ...
    def test_nouns_list__has_50_words(self):
        ...
        assert len(NOUNS_WARHAMMER) == 50, f"Expected 50 nouns, got {len(NOUNS_WARHAMMER)}"
    ...
    def test_nouns_list__no_duplicates(self):
        ...
        assert len(NOUNS_WARHAMMER) == len(set(NOUNS_WARHAMMER)), "NOUNS_WARHAMMER has duplicates"
