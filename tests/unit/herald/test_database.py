...
class TestUpdateExchangeWithListB:
    ...
    def test_update_exchange_with_list_b__success(self, create_test_exchange, test_engine):
        """Test successful update with list_b"""
        exchange_id = create_test_exchange(
            exchange_id="update-test",
            list_a="Player A",
            list_b=None
        )
    ...
    def test_update_exchange_with_list_b__only_updates_when_null(self, create_test_exchange):
...
class TestExchangeIsComplete:
    ...
    def test_exchange_is_complete__returns_true_when_complete(self, create_test_exchange):
    ...
    def test_exchange_is_complete__returns_false_when_pending(self, create_test_exchange):
...
class TestGetAbusiveIPs:
    ...
    def test_get_abusive_ips__returns_ips_over_threshold(self, test_engine):
        ...
        with test_engine.connect() as conn:
            for i in range(150):
                conn.execute(
                    text("INSERT INTO herald_request_log (ip, endpoint, user_agent) VALUES (:ip, :ep, :ua)"),
    ...
    def test_get_abusive_ips__filters_by_time_window(self, test_engine):
        ...
        with test_engine.connect() as conn:
            for i in range(200):
                conn.execute(
                    text("INSERT INTO herald_request_log (ip, endpoint, user_agent, timestamp) VALUES (:ip, :ep, :ua, :ts)"),
...
class TestDeleteOldExchanges:
    ...
    def test_delete_old_exchanges__deletes_old_only(self, test_engine):
        ...
        with test_engine.connect() as conn:
            conn.execute(
                text("INSERT INTO herald_exchanges (id, list_a, hash_a, timestamp_a, created_at) VALUES (:id, :la, :ha, :ta, :ca)"),
                {"id": "old-exchange", "la": "old", "ha": "hash", "ta": old_time, "ca": old_time}
            )
            conn.execute(
                text("INSERT INTO herald_exchanges (id, list_a, hash_a, timestamp_a, created_at) VALUES (:id, :la, :ha, :ta, :ca)"),
                {"id": "recent-exchange", "la": "recent", "ha": "hash", "ta": recent_time, "ca": recent_time}
            )
            conn.commit()
    ...
    def test_delete_old_exchanges__returns_count(self, test_engine):
        ...
        with test_engine.connect() as conn:
            for i in range(5):
                conn.execute(
                    text("INSERT INTO herald_exchanges (id, list_a, hash_a, timestamp_a, created_at) VALUES (:id, :la, :ha, :ta, :ca)"),
                    {"id": f"old-{i}", "la": "old", "ha": "hash", "ta": old_time, "ca": old_time}
                )
            conn.commit()
