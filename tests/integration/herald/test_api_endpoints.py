...
class TestGetExchangeEndpoint:
    ...
    def test_GET_exchange__returns_200_for_pending(self, test_client, create_test_exchange):
...
class TestRespondExchangeEndpoint:
    ...
    def test_POST_respond__404_for_invalid_id(self, test_client):
        ...
        response = test_client.post(
            "/api/herald/exchange/invalid/respond",
            json={"list_content": "B"}
        )
        
        assert response.status_code == 404
    
    def test_POST_respond__400_for_already_complete(self, test_client, create_test_exchange):
