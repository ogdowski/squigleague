            ...
            <div class="flex items-center gap-4">
              ...
              <div class="flex-1 min-w-0">
                <div class="flex flex-wrap gap-2">
                  ...
                  <button
                    v-if="hasDiscordOAuth"
                    type="button"
                    @click="syncDiscordAvatar"
                    :disabled="syncingDiscordAvatar"
                  ...
                  >
                    ...
                    {{ syncingDiscordAvatar ? t('common.loading') : t('settings.useDiscordAvatar') }}
                  </button>
                </div>
...
const syncingDiscordAvatar = ref(false)
...
const syncDiscordAvatar = async () => {
  syncingDiscordAvatar.value = true
  error.value = ''
  try {
    const response = await axios.post(`${API_URL}/auth/me/sync-discord-avatar`)
    ...
    error.value = err.response?.data?.detail || t('errors.failedToSyncDiscordAvatar')
  } finally {
    syncingDiscordAvatar.value = false
  }
}
