<template>
  <div class="max-w-md mx-auto">
    <div class="card">
      ...
      <div class="space-y-3">
        <!-- Google OAuth temporarily disabled during GCP verification -->
        <!-- <button
...
const loginWithGoogle = async () => {
  try {
    const response = await axios.get(`${API_URL}/auth/oauth/google`)
    window.location.href = response.data.authorization_url
  } catch (err) {
    error.value = 'Google login is not available at the moment'
  }
}
...
const loginWithDiscord = async () => {
  try {
    ...
    error.value = 'Discord login is not available at the moment'
  }
}
</script>
